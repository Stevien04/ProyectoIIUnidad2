...
export const AuditoriaReserva: React.FC<AuditoriaReservaProps> = ({ onAuditLog }) => {
  ...
  const cargarAuditorias = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8080/api/auditoria-reservas');
  ...
  const buscarConFiltros = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/auditoria-reservas/buscar?${params}`);
